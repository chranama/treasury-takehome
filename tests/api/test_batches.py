...
def test_disabled_live_backend_preserves_draft_and_rejects_batch_start(tmp_path: Path) -> None:
    settings = make_settings(
        tmp_path,
        extraction_backend="openai",
        live_extraction_enabled=False,
    ...
    with TestClient(create_app(settings)) as client:
        ...
        response = client.post(
            f"/api/batches/{created['batch_id']}/start",
            json={"selection": "all_cases"},
            headers={"Idempotency-Key": "disabled-live-batch-key"},
        )
        retained = client.get(f"/api/batches/{created['batch_id']}").json()

    assert response.status_code == 503
    assert response.json()["code"] == "batch_processing_unavailable"
    assert retained["state"] == "draft"
    assert retained["counts"]["ready"] == 1
    with connect(settings.database_path) as connection:
        assert connection.execute("SELECT COUNT(*) FROM review_submissions").fetchone() == (0,)
        assert connection.execute("SELECT COUNT(*) FROM provider_attempts").fetchone() == (0,)
